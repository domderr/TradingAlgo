...
def is_ticker(line):
    return bool(re.match(r"^[A-Z0-9][A-Z0-9.\-]{0,15}$", line))
...
def is_noise(line):
    ...
    return (
        not line
        or upper in SKIP_WORDS
        or bool(re.match(r"^\d+(?:\.\d+)?$", line))
        or "%" in line
        or line.startswith("(")
        or line.startswith("TradingAlgo")
    )


def extract_positions(text):
    """Positions appear before 'XX.XX% Capital Invested'."""
    ...
    for idx, line in enumerate(lines):
        legacy = re.match(r"^([A-Z0-9][A-Z0-9.\-]{0,15})\s*[-\u2013]\s*(.+)", line)
        if legacy:
            ticker = legacy.group(1).strip()
            name = legacy.group(2).strip()
            if is_ticker(ticker) and not is_noise(name):
                entries.append({"ticker": ticker, "name": name})
            continue

        if not is_ticker(line) or is_noise(line):
            continue

        name = lines[idx + 1].strip() if idx + 1 < len(lines) else ""
        if is_noise(name):
            continue
        entries.append({"ticker": line, "name": name})

    return entries[-5:]


def extract_changes(text):
    """Changes appear after the capital invested block: IN/OUT markers then tickers."""
    changes_in = []
    changes_out = []
    ...
    for idx, line in enumerate(lines):
        ...
        if line == "OUT":
            ...
            continue
        if not current_direction or line == "-":
            continue

        legacy = re.match(r"^([A-Z0-9.\-]{1,16})\s*[-\u2013]\s*(.+)", line)
        if legacy:
            entry = {"ticker": legacy.group(1), "name": legacy.group(2).strip()}
        elif is_ticker(line):
            name = lines[idx + 1].strip() if idx + 1 < len(lines) else ""
            if is_noise(name):
                continue
            entry = {"ticker": line, "name": name}
        else:
            continue
...
def main():
    ...
    for market in MARKETS:
        ...
        print(
            f"{market['key']}: {len(result[market['key']]['positions'])} positions, "
            f"in={len(result[market['key']]['changes']['in'])} "
            f"out={len(result[market['key']]['changes']['out'])}"
        )
